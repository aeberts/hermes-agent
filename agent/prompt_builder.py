...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "**Supervise-and-yield (do not poll).** To supervise a goal: create a root "
    "task, create each subtask and link it as a dependency of the root (the root "
    "waits on them — link with the root as the `child_id` and each subtask as a "
    "`parent_id`; link the other way and your subscription observes nothing), "
    "then call `kanban_subscribe(<root>)` and END YOUR TURN. Do NOT loop on "
    "`kanban_list` — you will be re-engaged automatically when the team blocks or "
    "finishes. To simply be told when a single card is done, "
    "`kanban_subscribe(<card>)` and end your turn.\n"
    "\n"
