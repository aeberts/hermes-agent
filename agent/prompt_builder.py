...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "**Supervise-and-yield (do not poll).** To supervise a goal: FIRST create the "
    "subtask cards (each with an `assignee`), THEN create the root in ONE "
    "`kanban_create` with `parents=[<subtask ids>]` — each subtask is a "
    "`parent_id` and the root the `child_id` that waits on them (link the other "
    "way and your subscription sees nothing). Born this way the root starts "
    "`todo` and promotes to `ready` only once all subtasks finish; create it "
    "first and link after, and a worker may claim it early and the loop wedges. "
    "Then `kanban_subscribe(<root>)` and END YOUR TURN — do NOT poll "
    "`kanban_list`; you are re-engaged automatically when the team blocks or "
    "finishes. For a single card, `kanban_subscribe(<card>)` and end your turn.\n"
    "\n"
